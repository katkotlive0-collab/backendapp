...
exports.likeUnlike = async (req, res, next) => {
  try {
    ...
    //like or unlike post
    if (req.body.postId) {
      const post = await Post.findById(req.body.postId).populate("userId", "fcmToken notification isBlock name image isVIP");
      if (!post)
        return res.status(200).json({
          status: false,
      ...
      if (likeExist.length === 0) {
        ...
        await favorite.save(async (error, favorite) => {
          if (error) return res.status(200).json({ status: false, error: error.message || "server error" });
          else {
            if (
              post.userId &&
    ...
    if (req.body.videoId) {
      ...
      if (likeExist.length === 0) {
        ...
        await Video.updateOne({ _id: video._id }, { $inc: { like: -1 } }).where({ like: { $gt: 0 } });
        await likeExist.map((data) => {
          data.deleteOne();
        });
        return res.status(200).json({
